 ...
 *
 *     CKEditor.plugin.add('jqueryspellcheck') - Means of interaction with the event modes
...
(function(window, $) {
  ...
  /* Character sets -> Used to determine valid text to send to the server on parse
   *************************/
  ...
  /* Events Extension to the standard jQuery events.  Built for notification on server callback / dom update.
   *
   */
  var Events = function(){
    this._handlers = {};
  ...
  Events.prototype = {
    on: function(name, handler) { //Bind a set of callbacks to the events
      if (!this._handlers[name]) {
        this._handlers[name] = $.Callbacks(); //Why does it do it in a callback stack?
      }
      this._handlers[name].add(handler);
    },
    trigger: function(name) { //Fire the event
  ...
  /* Handlers  (Click occured on a word that has an error)
   *************************/
...
console.log("Adding in the plugin.");
